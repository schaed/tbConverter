...
Event* StorageIO::readEvent(Long64_t n)
{
  ...
  for (unsigned int nplane = 0; nplane < _numPlanes; nplane++)
  {
    ...
    for (int nhit = 0; nhit < numHits; nhit++)
    {
      ...
      hit->setOffset(hitOffset[nhit]);
      hit->setSlope(hitSlope[nhit]);
...
void StorageIO::writeEvent(Event* event)
{
  ...
  for (unsigned int nplane = 0; nplane < _numPlanes; nplane++)
  {
    ...
    for (int nhit = 0; nhit < numHits; nhit++)
    {
      ...
      hitSlope[nhit] = hit->getSlope();
      hitResetTime[nhit] = event->getResetTime();
      hitOffset[nhit] = hit->getOffset();
  ...
  _noiseMasks(0)
{
  ...
  if (_fileMode == OUTPUT)
  {
    ...
    for (unsigned int nplane = 0; nplane < _numPlanes; nplane++)
    {
      ...
      hits->Branch("Offset", hitOffset, "HitOffset[NHits]/D");
      hits->Branch("Slope", hitSlope, "HitSlope[NHits]/D");
      hits->Branch("ResetTime", hitResetTime, "HitResetTime[NHits]/D");
    ...
    _eventInfo->Branch("ResetTime", &resetTime, "ResetTime/D");
  ...
  if (_fileMode == INPUT)
  {
    ...
    while (true)
    {
      ...
      if (hits)
      {
        ...
        hits->SetBranchAddress("Offset", hitOffset, &bHitOffset);
        hits->SetBranchAddress("Slope", hitSlope, &bHitSlope);
        hits->SetBranchAddress("ResetTime", hitResetTime, &bHitResetTime);
      }
    ...
    if (_eventInfo)
    {
      ...
      _eventInfo->SetBranchAddress("ResetTime", &resetTime, &bResetTime);
